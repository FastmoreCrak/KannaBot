...
import Kanna.modules.sql.feds_sql as sql
from Kanna import (GBAN_LOGS, LOGGER, OWNER_ID, SUDO_USERS, TIGER_USERS,
                          WHITELIST_USERS, dispatcher)
from Kanna.modules.disable import DisableAbleCommandHandler
from Kanna.modules.helper_funcs.alternate import send_message
from Kanna.modules.helper_funcs.chat_status import is_user_admin
from Kanna.modules.helper_funcs.extraction import (extract_unt_fedban,
                                                          extract_user,
                                                          extract_user_fban)
from Kanna.modules.helper_funcs.string_handling import markdown_parser
...
def new_fed(update: Update, context: CallbackContext):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Can't federate! Please contact {SUPPORT_CHAT} if the problem persist."
            )
            return
...
def join_fed(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            message.reply_text(
                "Failed to join federation! Please contact {SUPPORT_CHAT} should this problem persist!"
            )
            return
...
def fed_ban(update: Update, context: CallbackContext):
    ...
    if fban:
        ...
        if not x:
            message.reply_text(
                "Failed to ban from the federation! If this problem continues, contact {SUPPORT_CHAT}."
            )
            return
...
def set_frules(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Whoa! There was an error while setting federation rules! If you wondered why please ask it in {SUPPORT_CHAT} !"
            )
            return
