...
from Kanna.modules.helper_funcs.handlers import (CMD_STARTERS)
from Kanna.modules.helper_funcs.misc import is_module_loaded
...
if is_module_loaded(FILENAME):
    ...
    from Kanna.modules.helper_funcs.chat_status import (
        connection_status, is_user_admin, user_admin)
    from Kanna.modules.sql import disable_sql as sql
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text(
                    f"Inhabilitado el uso de `{disable_cmd}`",
                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text(
                    "Ese comando no se puede desactivar")

        else:
            update.effective_message.reply_text("Que debo deshabilitar?")
    ...
    def disable_module(update: Update, context: CallbackContext):
        ...
        chat = update.effective_chat
        if len(args) >= 1:
            disable_module = "Kanna.modules." + args[0].rsplit(".", 1)[0]
            ...
            if failed_disabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"Los comandos `{failed_disabled_cmds_string}` no se pueden desactivar",
                    parse_mode=ParseMode.MARKDOWN)
    ...
    def enable(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text(
                    f"Habilitado el uso de `{enable_cmd}`",
                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("Esta aun desabilitado?")
    ...
    def enable_module(update: Update, context: CallbackContext):
        ...
        if len(args) >= 1:
            enable_module = "Kanna.modules." + args[0].rsplit(".", 1)[0]
            ...
            if failed_enabled_cmds:
                ...
                update.effective_message.reply_text(
                    f"Están los comandos '{failed_enabled_cmds_string}' incluso deshabilitados?",
                    parse_mode=ParseMode.MARKDOWN)
    ...
    def list_cmds(update: Update, context: CallbackContext):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text(
                f"Los siguientes comandos se pueden combinar:\n{result}",
                parse_mode=ParseMode.MARKDOWN)
        else:
    ...
    def __stats__():
        return f"{sql.num_disabled()} items deshabilitados, a través de {sql.num_chats()} chats."
