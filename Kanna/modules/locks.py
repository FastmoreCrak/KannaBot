...
import Kanna.modules.sql.locks_sql as sql
from Kanna import dispatcher, SUDO_USERS, LOGGER
...
from Kanna.modules.log_channel import loggable
from Kanna.modules.connection import connected

from Kanna.modules.helper_funcs.alternate import send_message, typing_action
        ...
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                if conn:
                    ...
                    text = "Se bloquearon todos los {} mensajes para los miembros.".format(
                        ltype)
                ...
                send_message(
                    update.effective_message,
                    "¿Qué estás intentando bloquear...? Pruebe /locktypes para la lista de bloqueables",
                )
        else:
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            ...
            if ltype in LOCK_TYPES:
                ...
                send_message(
                    update.effective_message,
                    "¿Qué estás intentando desbloquear ...? Pruebe /locktypes para la lista de bloqueables.",
                )
...
def list_locks(update, context):
    ...
    if conn:
        res = res.replace("Bloquear en", "*{}*".format(chat_name))
...
* Solo administrador: *
 × /lock <tipo>: Bloquea elementos de un determinado tipo (no disponible en privado)
